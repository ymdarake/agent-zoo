...
> 📋 **作業計画書**: [2026-04-18 統合作業ブレークダウン](../../plans/2026-04-18-consolidated-work-breakdown.md) — 本レポートの指摘を Sprint 004〜008 / PR 単位に落とし込んだ実装プラン。
...
#### C-1 (Claude=Critical, Gemini=High に修正提案). dashboard が Werkzeug デバッグモードで起動 — ✅ Sprint 005 PR B で resolved（`FLASK_DEBUG=1` 撤去、gunicorn に戻す）
...
#### C-2 (Gemini G-1, NEW Critical). mitmproxy addon の未捕捉例外でポリシーバイパス（fail-open）— ✅ Sprint 005 PR A で resolved（[ADR 0005](../adr/0005-fail-closed-addons.md)）
...
#### H-1. dashboard 全 POST に CSRF 対策が無い — ✅ Sprint 005 PR B で resolved（Flask-WTF CSRFProtect 導入）
...
#### H-2. inbox reject／bulk-reject に path traversal の余地 — ✅ Sprint 005 PR B で resolved（strict regex + `path.resolve().is_relative_to` 2 段防御）
...
#### H-4. agent が任意 file 名で `/harness/inbox/*.toml` を書け、dashboard 側で属性 injection 余地 — ✅ Sprint 005 PR B で resolved（glob stem filter + `hx-vals` `|tojson|forceescape` + CSP）
...
#### M-1. dashboard が外部 CDN を SRI 無しで読み込み ✅ Sprint 007 (PR F〜I) で resolved
...
**Sprint 007 (PR F〜I) 解決**: ADR 0004 に基づき pico/htmx を **完全撤去**、自前 HTML/CSS/vanilla JS に書換。CDN ドメインへの参照が template / CSP の双方から消え、自前 `static/app.css` (~284 行) + `static/app.js` (~268 行) のみで動作。CSP も `'self'` only に厳格化 (`'unsafe-inline'` / CDN ドメイン全削除、`form-action 'self'` 追加)。Playwright で CDN を route block しても dashboard 全機能 PASS (`tests/e2e/test_dashboard_offline.py`)。

#### M-2. log DB に Authorization ヘッダ／クエリ token 含む URL が永続化 ✅ Sprint 006 PR D で resolved
...
**修正 (実施済)**: `bundle/addons/_url_scrub.py::scrub_url` で userinfo / query / fragment を redact、host を lowercase 正規化、CR/LF/制御文字混入は `[invalid-url]` 化。`PolicyEngine.check_url_secrets` で secret_patterns を URL に適用、検出時は 403 `URL_SECRET_BLOCKED` で遮断。詳細は `docs/dev/security-notes.md`。

#### M-3. Dockerfile base が mutable tag ✅ Sprint 006 PR E で resolved
...
**修正 (実施済)**: 全 4 image を multi-arch manifest list digest で pin。`@sha256:...` 付き `FROM` / `image:` に書換、`.github/dependabot.yml` の docker ecosystem で週次更新 PR を運用。新 SHA 受入時の manifest list 検証手順は `docs/dev/security-notes.md` を参照。

#### M-4. GitHub Actions third-party action が tag pinning（SHA pin 推奨） ✅ Sprint 006 PR E で resolved
...
**修正 (実施済)**: ci.yml + release.yml の 19 uses 行を全部 commit SHA pin、`# <tag>` コメント併記で Dependabot が SemVer 追跡可能に。`pypa/gh-action-pypi-publish` は branch SHA から tag SHA (`v1.14.0`) に切替済 (branch ref のままでは Dependabot が更新 PR を作れないため)。

#### M-5. `_validate_domain` の正規表現が緩い ✅ Sprint 006 PR D で resolved
...
**修正 (実施済)**: `_LABEL_RE = (?!-)[A-Za-z0-9-]{1,63}(?<!-)` で RFC 1035 準拠 strict 化、`*.` 後は 2 ラベル以上強制。inbox accept 経由の bypass も `_apply_accept` で同 regex 適用済。既存 `bundle/policy.toml` 17 entries は全部通過確認 (回帰テスト)。behavior change は CHANGELOG `### Changed` に明記。

#### M-6. mitmproxy `body_size_limit` 未設定 → 巨大 payload で memory 圧迫 ✅ Sprint 006 PR D で resolved
...
**修正 (実施済)**: docker-compose proxy command に `--set body_size_limit=1m` 追加 (OOM 保護)。さらに addon 側で Content-Length > 1MB を 413 で先行遮断 (fail-closed、stream pass-through で secret_patterns 検査が silently bypass される副作用を塞ぐ)。Plan review (Gemini 3 + Claude subagent) で fail-open 副作用が指摘され 2 段防御に強化。

#### M-7. `tool_use_rules.block_args` がワード境界マッチで bypass 容易 ✅ Sprint 006 PR D で resolved
...
**修正 (実施済)**: `docs/user/policy-reference.md` / `.en.md` に抽象 warning 追加、`docs/dev/security-notes.md` (新設、dev-only) に bypass 例を分離（agent self-jailbreak 防御）。`bundle/policy.toml` の block_args コメントは中立表現に書換。

#### M-8 (Gemini G-3). `policy.toml` の TOCTOU 競合状態 ✅ Sprint 006 PR F で resolved
...
**修正方針**: enforcer 側 read 時に `fcntl.LOCK_SH` を取得し、edit 側 `LOCK_EX` と協調。

**defer 理由 (Sprint 006 PR D Plan Review で発覚)**: proxy container は `/config/policy.runtime.toml:ro` で ro mount され、`/config/policy.runtime.toml.lock` を open("w") できず EROFS。cross-container 共有 lock dir (`./locks:/locks` rw mount) と `_policy_lock_path()` helper の新設が必要で、独立 PR (Sprint 006 PR F) で対応する。当面は `atomic_write` の tmpfile + rename で partial read は防げており実害小。

**修正 (実施済 Sprint 006 PR F)**: `bundle/addons/_policy_lock.py` 新設で `policy_lock_shared` (reader) / `policy_lock_exclusive` (writer) を分離 API として提供。reader は warn + passthrough (ADR 0005 fail-closed と両立)、writer は raise (fail-closed)。proxy / dashboard に `./locks:/locks` bind mount を追加、`zoo init` で `.zoo/locks/` を自動生成。lock file は `O_NOFOLLOW + 0o600` で symlink 攻撃を抑止。`policy_edit.policy_lock` は新 helper の thin alias。詳細: `docs/dev/security-notes.md`。
...
- **L-6**: `inbox.html:28, 32` の `'` クォート属性内未エスケープ（H-4 と関連）。 ✅ H-4 (Sprint 005 PR B) で `|tojson|forceescape` 適用済 + Sprint 007 PR I で CSP `'self'` only により script 注入経路完全封鎖の 2 段で resolved。
...
#### G3-B1. 監査ログ (`harness.db`) による機密情報 (PII) 蓄積（Medium-High） ✅ Sprint 006 PR D で部分 resolved (chmod 600)
...
**修正方針**:
- `general` セクションに `body_capture_exclude_domains` を追加し、機微 API への request body 記録を停止 → **未実施 (ROADMAP)**
- DB 暗号化 (sqlcipher 等) は重いので opt-in、最低でも file 権限 600 を強制 → ✅ **Sprint 006 PR D で実施**: `bundle/addons/_db_secure.py::secure_db_file` で db / db-wal / db-shm を chmod 600。symlink follow 抑止、bind mount EPERM は fail-safe。WAL rotation で剥がれる既知制約は `docs/dev/security-notes.md`。

`body_capture_exclude_domains` は ROADMAP として残置（Sprint 008 以降）。
