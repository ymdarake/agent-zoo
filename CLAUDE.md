...
```bash
# コンテナモード（ANTHROPIC_AUTH_TOKEN 必須）
ANTHROPIC_AUTH_TOKEN=xxx make run              # 対話実行
ANTHROPIC_AUTH_TOKEN=xxx make task PROMPT="..." # 自律実行
...
Phase 1-3 実装済み。ユニットテスト124件、Dockerスモークテスト。
...
- 認証: `ANTHROPIC_AUTH_TOKEN` を毎回環境変数で渡す。.envファイルは使わない
