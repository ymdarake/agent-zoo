...
- **ホットリロード**: policy.toml更新で反映（ホストモードは即時、コンテナモードは`docker compose restart proxy`）
...
# 2. 対話モードで起動（初回はコンテナ内で /login が必要。WORKSPACEで作業ディレクトリ指定）
WORKSPACE=/path/to/my-project make run

# 3. 自律実行モードで起動（CLAUDE_CODE_OAUTH_TOKEN で認証）
...
ホストモードではファイル保存で即反映。コンテナモードではDocker bind mountの制約により`docker compose restart proxy`が必要。
...
[tool_use_rules]
# 危険なツール実行をリアルタイムで阻止（独立条件）
block_args = ["rm -rf /", "chmod 777", "printenv", "/etc/shadow"]

# 組み合わせ条件でブロック（ルール内AND、ルール間OR）
...
3. make analyze → Claude CLIがブロックログを分析し、policy.toml改善をTOML形式で提案
   ...
   ↓
4. policy.toml更新 → ホストモードは即反映、コンテナモードは `docker compose restart proxy`
   ↓
