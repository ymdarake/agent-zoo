...
- `bundle/addons/policy_enforcer.py` で `flow.request.url` 保存前に `urllib.parse.urlsplit` → query + fragment + userinfo を `[redacted]` 化（scrub_url）
- 関連: `secret_patterns` を URL にも適用（常時 ON、policy.toml への設定追加はなし。詳細は [docs/plans/sprint-006-pr-d.md](sprint-006-pr-d.md) M-2 節）
- **追加**: Content-Length > 1MB request は addon 側で 413 に fail-close（M-6 と融合）
- テスト: `?api_key=secret` 付き URL が DB から scrub、secret_patterns で block されること
...
##### P006D-5. ~~`policy_enforcer.load_policy` に LOCK_SH（M-8）~~ → **Sprint 006 PR F に分離**

PR D Rev.2 レビューで、proxy container の `/config/` ro bind mount と `fcntl.LOCK_SH` 取得用 lockfile の書き込み要件が衝突することが判明（`/config/policy.runtime.toml.lock` を open("w") で EROFS）。cross-container 共有可能な lock dir 設計 (e.g., `./locks:/locks` rw mount) と `_policy_lock_path()` 共通化が必要で、独立 PR として Sprint 006 PR F で対応する。詳細は [docs/plans/sprint-006-pr-d.md](sprint-006-pr-d.md) の「H1 の deferred 理由」節。

現行 atomic rename で partial read は防げており、PR F 完了まで TOCTOU は実害小。
...
- [ ] M-2, M-5, M-6, M-7, G3-B1 の 5 項目反映（M-8 は PR F に defer）
- [ ] 新規テスト 35 件（scrub / validate / db perm / content-length / url-secret）
- [ ] 全 unit + E2E PASS
...
### PR F: policy_lock 共通化 + LOCK_SH（M-8、PR D から defer）

#### 背景

PR D Rev.2 レビューで発覚した「proxy container の `/config/` ro bind mount と `fcntl.LOCK_SH` lockfile 書き込み要件の衝突」を解決する独立 PR。

#### 作業単位

##### P006F-1. `bundle/addons/_policy_lock.py` 新設

- `_policy_lock_path(policy_path) -> str`: policy file に対応する lock file path を共有 lock dir 下に mapping
- `policy_lock_shared(policy_path)` / `policy_lock_exclusive(policy_path)` context manager
- `POLICY_LOCK_DIR` env (default: `/locks`)

##### P006F-2. `bundle/docker-compose.yml` に `./locks:/locks:rw` bind mount

- proxy / dashboard 両方に追加
- host 側 `./locks/` を `.gitignore` 追加

##### P006F-3. 既存 `policy_edit.py::policy_lock` の新 helper 移行

- `add_to_allow_list` / `add_to_dismissed` / `add_to_paths_allow` / `remove_from_allow_list` / `remove_from_paths_allow` / `remove_from_dismissed` の 6 箇所

##### P006F-4. `policy.py::PolicyEngine._load` に LOCK_SH

- 冒頭で `policy_lock_shared(policy_path)` コンテキストマネージャを取得
- `_runtime_path` の load も同様

##### P006F-5. テスト

- `tests/test_policy_lock.py`: 協調テスト (concurrent LOCK_SH + LOCK_EX)
- lock dir 不在時の fallback
- EROFS / EPERM 時の fail-safe

#### 受入基準（PR F）

- [ ] 全 6 callsite が新 helper に移行
- [ ] proxy + dashboard で `/locks` rw mount 成立
- [ ] 新規テスト 5+ 件 PASS
- [ ] E2E P2 (`test_proxy_block.py`) が maybe_reload 経路込みで green
- [ ] 包括レビュー M-8 resolved マーク
