...
3 mode あり、通常は Makefile 経由で叩く:

```bash
# branch-protected main (PR 必須環境) — 2-phase flow
make release-commit 0.1.1b1    # phase 1: pyproject bump + commit のみ (release branch 用)
#   ...PR → squash merge → git checkout main && git pull...
make release-tag 0.1.1b1       # phase 2: main HEAD に annotated tag のみ

# legacy (branch protection 無し、直接 push 可能な repo 用)
make release 0.1.0b1           # 全部入り: bump + commit + tag
...
./scripts/release-prepare.sh --dry-run [mode] ...
```

実施内容 (共通):

- PEP 440 native public version のみ accept (`X.Y.Z` / `X.Y.Z(a|b|rc)N`、leading zero reject)
- working tree clean + branch 確認 (非 TTY / `CI=*` 環境で main 以外は abort、ただし `--no-tag` は release branch 前提で main 以外でも通る)
- local / origin の tag `v<VERSION>` 未存在 check
- `pyproject.toml` の `[project].version` を section-aware に書き換え (`[tool.foo].version` 等を誤爆しない)、`tomllib` で read-back verify
- `:bookmark: release: v<VERSION>` commit + annotated tag (mode で on/off)。失敗 / SIGINT で rollback
- `--tag-only` は pyproject.version == VERSION + HEAD subject `release: v<VERSION>` + HEAD == origin/main の 3 precondition を fail-fast で check (orphan tag による本番 PyPI 暴発を防止)
- push は意図的に行わず、次手順を echo
