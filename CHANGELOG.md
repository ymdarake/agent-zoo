...
- **バージョンアップ作業の自動化 (`make release-commit` + `make release-tag` 2-phase flow)** ([#68](https://github.com/ymdarake/agent-zoo/issues/68)) — main branch protection (PR 必須) 運用に最適化。`release-commit` (`--no-tag`) は release branch で pyproject bump + `:bookmark: release: v<VERSION>` commit を作成 (tag は打たない)、`release-tag` (`--tag-only`) は squash merge 後の main HEAD に annotated tag を打つ。`release-tag` は **pyproject.version == VERSION + HEAD subject `release: v<VERSION>` + HEAD == origin/main** の 3 precondition を fail-fast で verify し、orphan tag による本番 PyPI 暴発を防ぐ。`[project].version` は Python で section-aware に書き換え (`[tool.foo].version` 等を誤爆しない)、`tomllib` で read-back verify、失敗 / SIGINT で rollback。spec regex は `release.yml` classify と同一 (drift 防止 test 付き)。push は意図的に自動化せず next step を echo。各 mode に `-dry-run` 版あり
- **`.github/PULL_REQUEST_TEMPLATE/release.md` 新設** — release PR 専用 checklist。`gh pr create --template release.md` で呼び出し、CHANGELOG / pyproject version 整合 / pre-release vs stable 判定 / TestPyPI yank / Trusted Publisher 設定 / rollback 手順を忘れず確認できるようにする。`scripts/release-prepare.sh --no-tag` の next step echo でも案内される
