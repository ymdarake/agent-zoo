...
`api.anthropic.com`への通信は許可しているため、会話コンテキストはAnthropicに送信される。
...
コンテナ内で`printenv`や`/proc/1/environ`で環境変数は読めるが、ネットワーク隔離で送れない。`block_patterns`に`printenv`をデフォルト設定済み。
