...
- **`make release <VERSION>` / `make release-dry-run <VERSION>`** ([#68](https://github.com/ymdarake/agent-zoo/issues/68)) — バージョンアップ作業 (pyproject.toml bump + `:bookmark: release: v<VERSION>` commit + `v<VERSION>` tag) を 1 発で自動化する maintainer 用 Makefile target (`scripts/release-prepare.sh`)。release.yml classify と同一 spec の PEP 440 native regex で事前検証、working tree clean + branch 確認 (非 TTY / `CI=*` 環境で main 以外は abort)、`[project].version` を Python で section-aware に書き換え (`[tool.foo].version` 等を誤爆しない)、失敗 / SIGINT で rollback。push は意図的に自動化せず next step を echo
- **`make release-commit <VERSION>` / `make release-tag <VERSION>` — branch-protected main 向けの 2-phase release flow** — main protection で直接 push 不可な環境で、release を PR 経由に分離。`release-commit` (`--no-tag`) は pyproject bump + commit のみを release branch で作成、`release-tag` (`--tag-only`) は squash merge 後の main HEAD に annotated tag を打つ。`release-tag` は **pyproject.version == VERSION + HEAD subject `release: v<VERSION>` + HEAD == origin/main** の 3 precondition を fail-fast で verify し、orphan tag による本番 PyPI 暴発を防ぐ。各 mode に `-dry-run` 版あり。従来の `make release` 全部入り target は branch protection 無し repo 用の legacy として残存 (42 unit test、合計 9 新規 + 33 既存)
