...
| `Makefile` | `bundle/Makefile` (maintainer 用に維持) | **配布しない** |
...
maintainer は **2 通り**で開発:
1. **source repo の `bundle/` 内で `make` を直接叩く** — `cd bundle && make build` 等
2. **別 dir で zoo init** — `pip install -e .` → 別 dir で `zoo init` → 新 layout で smoke
...
# bundle/Makefile は配布しない（maintainer 開発用）
```
