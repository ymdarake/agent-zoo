...
- **`make release <VERSION>` / `make release-dry-run <VERSION>`** ([#68](https://github.com/ymdarake/agent-zoo/issues/68)) — バージョンアップ作業 (pyproject.toml bump + `:bookmark: release: v<VERSION>` commit + `v<VERSION>` tag) を 1 発で自動化する maintainer 用 Makefile target (`scripts/release-prepare.sh`)。release.yml classify と同一 spec の PEP 440 native regex で事前検証、working tree clean + branch 確認 (非 TTY / `CI=*` 環境で main 以外は abort)、`[project].version` を Python で section-aware に書き換え (`[tool.foo].version` 等を誤爆しない)、失敗 / SIGINT で rollback。push は意図的に自動化せず next step を echo。32 unit test (shell injection defense-in-depth + regex drift 防止 + empty anchor commit note 等)
