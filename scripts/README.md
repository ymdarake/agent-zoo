...
```bash
make release-dry-run 0.1.0b1    # 副作用なし事前検証
make release 0.1.0b1            # 本実行 (push は手動)
```

直接呼ぶ場合:

```bash
./scripts/release-prepare.sh --dry-run 0.1.0b1
./scripts/release-prepare.sh 0.1.0b1
```

実施内容:

- PEP 440 native public version のみ accept (`X.Y.Z` / `X.Y.Z(a|b|rc)N`、leading zero reject)
- working tree clean + branch 確認 (非 TTY / `CI=*` 環境で main 以外は abort)
- `pyproject.toml` の `[project].version` を section-aware に書き換え (`[tool.foo].version` 等を誤爆しない)、`tomllib` で read-back verify
- `:bookmark: release: v<VERSION>` commit + `v<VERSION>` tag。失敗 / SIGINT で rollback
- push は意図的に行わず、次手順を echo (destructive action は maintainer 明示確認に分離)
